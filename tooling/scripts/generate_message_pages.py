...
class AnomalyLog:
    ...
    def add(self, category, message_id, detail):
        """Record an anomaly.

        Args:
            category: Short classification (e.g. "ADOC_CONFLICT")
            message_id: The Message ID involved (e.g. "QBP-Q11"), or None
            detail: Human-readable explanation of why this needs attention
        """
        self.entries.append({
            'category': category,
    ...
    def write(self, filepath):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write('=' * 72 + '\n')
            f.write('ANOMALY LOG — generate_message_pages.py\n')
            ...
            for cat in sorted(categories.keys()):
                ...
                f.write('-' * 72 + '\n\n')
                for item in sorted(items, key=lambda x: x['message_id']):
                    f.write(f'  {item["message_id"]}:\n')
                    # Wrap detail text for readability
...
# Metadata attribute lines to strip from AsciiDoc content
...
def html5_to_xhtml(html):
    ...
    # Downshift headings: h1->h3, h2->h4, h3->h5, etc.
    ...
    # Fix overlapping sup/sub tags that Asciidoctor can produce from
    # AsciiDoc inline formatting. E.g.:
    #   <sup>X<sub>Y</sup>Z</sub> → <sup>X<sub>Y</sub></sup><sub>Z</sub>
    html = re.sub(
        r'<sup>([^<]*)<sub>([^<]*)</sup>([^<]*)</sub>',
