...
[segment-definition]
This field contains the character set for the entire message. Refer to {{v2-table:0211:Alternate Character Sets}} in Chapter 2C, Code Tables, for valid values.

[segment-comment]
An HL7 message uses field MSH-18 Character Set to specify the character set(s) in use. Valid values for this field are specified in {{v2-table:0211:Alternate Character Sets}}. MSH-18 Character Set MAY be left blank, or MAY contain one or more values delimited by the repetition separator. If the field is left blank, the character set in use is understood to be the 7-bit ASCII set, decimal 0 through decimal 127 (hex 00 through hex 7F). This default value MAY also be explicitly specified as *ASCII*.
