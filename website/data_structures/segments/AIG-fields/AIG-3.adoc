== AIG.3 - Resource ID (CWE)

[segment-definition]
This field contains the ID number and name of the resource being requested or scheduled for an appointment. This field is used to identify a specific resource being requested, or a specific resource that has been scheduled for an appointment. If the specific resource is not known but the type of resource is, _AIG-4 Resource Type_ is used to identify the type of resource required or scheduled.
