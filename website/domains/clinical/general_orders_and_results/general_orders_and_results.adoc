...
This topic covers messaging for general orders and results. As described below, specific use cases use dedicated message types and events to fulfill their functional requirements. Dedicated topics have been created elsewhere is this specification to describe those specific structures.
...
=== Documents versus Results

If the observation being reported meets one or more of the following criteria, then the content would qualify as a medical document management message (MDM) rather than an observation message (ORU). The reader is referred to the definition of the MDM message structures in the Medical Records topic.
...
• Documents/reports where the Sender wants to indicate the availability of the report for use in patient care using the availability status present in the TXA segment.
...
• Documents/reports where the content as a whole requires special confidentiality protection using the confidentiality status present in the TXA segment.

• Documents/reports where document storage status is useful for archival and purging purposes using the storage status present in the TXA segment.

Using these criteria, the following examples of documents/reports would typically qualify as MDM messages. Note that as clinical content, the following documents/reports typically require succession management and/or report availability thus would require an MDM message even if the payload utilizes CDA.
...
=== Associations between Order Control Codes and Trigger Events
[v291_section="4.18.1"]
...
Order Control Codes / Trigger Event Matrix
...
FIXME - either remove the note below or do something about it (see outstanding question in GitHub about the content of this table)
...
Chapter 2, Section 2.10.4 defines the meaning of snapshot mode updates and indicates that each topic or related implementation guide may further refine this definition. The following guidance applies to results messages:
...
MSH|...<cr>
PID|...<cr>
ORC|NW|A226677^PC||946281^PC||N|3^QAM||202601121132|444-44-4444^HIPPOCRATES^HAROLD^^^^MD|||4EAST|...<cr>  // EKG order
OBR|1|||8601-7^EKG IMPRESSION^LN||||||||||||222-33-4444^PUMP^PATRICK^^^^MD|||||||||||3^QAM|...<cr>
BLG|...<cr>
ORC|NW|...<cr>  // Another order yet others may follow
....
...
ORC|CH|A226677^PC|89‑551^EKG|946281^PC|SC|||A226677&PC^89-458&EKG|...|^^^^202601130500^...<cr>  // 1ST child ORC
OBR|1||89‑551^EKG|8601-7^EKG IMPRESSION^LN|...<cr>  // 1ST child OBR
ORC|CH|A226677^PC|89-522^EKG|946281^PC|SC|||A226677&PC^89-458&EKG|...|^^^^20261140500^...<cr>  // 2ND child ORC
OBR|2||89‑552^EKG|8601-7^EKG IMPRESSION^LN|...<cr>  // 2ND child OBR
ORC|CH|A226677^PC|89‑553^EKG|946281^PC|SC|||A226677&PC^89-458&EKG|...|^^^^20261150500^...<cr>  // 3RD child ORC
OBR|3||89‑553^EKG|8601-7^EKG IMPRESSION^LN|... <cr>  // 3RD child OBR
...
MIC and disk diffusion (Kirby Bauer) susceptibility results can be combined in the same OBX segment. An OBX can contain a MIC value (in _OBX-5 Observation Value_ (results)) and _OBX-8 Interpretation Codes_ that indicates whether the organism is sensitive, resistant, or intermediate (see {{v2-table:0078:Interpretation Codes}} under abnormal flag fields).
